...
my $usage=<<INFO;
    ...
    Optional parameters
        ...
        --snv_min_vaf           default=2.5     Minimum VAF allowed to count a SNV/small indel (<5bp) as detected in a normal sample
        --snv_min_readcount     default=3       Minimum number of variant reads to count a SNV/small indel (<5bp) as detected in a normal sample
        --snv_min_recurrence    default=5       Minimum number of normal samples where a SNV/small indel (<5bp) is detected for it to fail this filter
        --indel_min_vaf         default=2.5     Minimum VAF allowed to count a 5+ bp indel as detected in a normal sample
        --indel_min_readcount   default=2       Minimum number of variant reads to count a 5+ bp indel as detected in a normal sample
        --indel_min_recurrence  default=7       Minimum number of normal samples where a 5+ bp indel is detected for it to fail this filter
...
print "Using indel (5+ bp) minimum VAF: $indel_VAF\n";
print "Using indel (5+ bp) minimum readcount: $indel_readcount\n";
print "Using indel (5+ bp) minimum recurrence count: $indel_recurrence\n";
...
while(my $line = <$norm>) {
    ...
    if($var[0] =~ /^chr/) {
        ...
        if($var[6] eq "NA") {
            ...
            if($var[3] =~ /-|0/ || $var[4] =~ /-|0/) {
                ## Check if the variant is a small indel (<5bp)
                if(length($var[3])<5 && length($var[4])<5){
